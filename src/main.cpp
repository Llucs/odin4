...
#define ODIN4_VERSION "5.0.2-8617c11"

static void print_usage() {
  std::cout << "Usage: odin4 [options]" << std::endl;
  std::cout << "Samsung firmware flashing tool for Linux. Version: "
            << ODIN4_VERSION << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  -l                  List detected Download Mode devices"
            << std::endl;
  std::cout << "  -d <path>            Select a specific USB device path (e.g. "
               "/dev/bus/usb/001/002)"
            << std::endl;
  ...
  std::cout << "  -u <file>            UMS archive (.tar or .tar.md5)"
            << std::endl;
  std::cout << "  --check-only         Validate PIT + archives and exit "
               "without flashing"
            << std::endl;
  std::cout << "  --allow-unknown      Allow archive entries without a PIT "
...
static bool has_any_firmware_files(const OdinConfig &cfg) {
  return !cfg.bootloader.empty() || !cfg.ap.empty() || !cfg.cp.empty() ||
         !cfg.csc.empty() || !cfg.ums.empty();
}
...
static void list_devices(const UsbSelectionCriteria &criteria) {
  const std::vector<std::string> devices =
      UsbDevice::list_download_devices(criteria);
  if (devices.empty()) {
...
static bool verify_firmware_compatibility(const OdinConfig &cfg,
                                          const std::string &device_type) {
  if (device_type.empty()) {
    log_verbose(
        "Device type is empty; skipping filename compatibility checks.");
    return true;
  }
  ...
  for (const std::string &p :
       {cfg.bootloader, cfg.ap, cfg.cp, cfg.csc, cfg.ums}) {
    if (p.empty())
      continue;
    if (!file_matches(p)) {
      log_error("Firmware file name does not appear to match device type: " +
                std::filesystem::path(p).filename().string() + " vs " +
                device_type);
      return false;
    }
  }
...
static void print_access_hint() {
  log_error("Permission denied while opening the USB device. This usually "
            "means udev rules are missing.");
...
static ExitCode run_for_device(const OdinConfig &cfg) {
  ...
  if (!usb.open_device(cfg.device_path, criteria)) {
    if (usb.get_last_open_error() == UsbOpenError::AccessDenied) {
      ...
      log_error(
          "Device detected, but it does not appear to be in Download Mode.");
    } else {
      log_error("No compatible device could be opened. Ensure the device is in "
                "Download Mode and try again.");
    }
    return ExitCode::Usb;
  }
  ...
  log_info("PIT received with " + std::to_string(pit.entries.size()) +
           " entries.");
  ...
  if (has_any_firmware_files(cfg)) {
    ...
    for (const auto &item : archives) {
      if (item.second.empty())
        continue;
      ExitCode rc = process_tar_file(item.second, usb, pit, !cfg.dry_run,
                                     cfg.allow_unknown);
...
static ExitCode process_arguments_and_run(int argc, char **argv) {
  ...
  if (cfg.dry_run && !has_any_firmware_files(cfg)) {
    log_error(
        "--check-only requires at least one firmware archive (-b/-a/-c/-s/-u)");
    return ExitCode::Usage;
  }
...
int main(int argc, char **argv) {
  ...
  if (err < 0) {
    std::cerr << "Failed to initialize libusb: " << libusb_error_name(err)
              << std::endl;
    std::cerr << "This may indicate that libusb is missing or incompatible on "
                 "this system."
              << std::endl;
