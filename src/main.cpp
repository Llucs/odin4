...
static std::string to_lower_copy(std::string t) {
    std::transform(t.begin(), t.end(), t.begin(), [](unsigned char c){ return (char)std::tolower(c); });
    return t;
}

std::string sanitize_filename(const std::string& filename) {
    // Remove extensions in a case-insensitive manner.
    std::string sanitized = filename;

    // Remove known multi-extensions (e.g. .lz4, .ext4, .img, .bin), any case.
    while (true) {
        auto dot = sanitized.find_last_of('.');
        if (dot == std::string::npos) break;
        std::string ext = to_lower_copy(sanitized.substr(dot));
        if (ext == ".lz4" || ext == ".ext4" || ext == ".img" || ext == ".bin") {
            sanitized = sanitized.substr(0, dot);
            continue;
        }
        break;
    }

    // Remove .tar / .md5 suffixes (also case-insensitive)
    std::string lower = to_lower_copy(sanitized);
    if (lower.size() >= 4 && lower.substr(lower.size() - 4) == ".tar") {
        sanitized = sanitized.substr(0, sanitized.size() - 4);
        lower = to_lower_copy(sanitized);
    }
    if (lower.size() >= 4 && lower.substr(lower.size() - 4) == ".md5") {
        sanitized = sanitized.substr(0, sanitized.size() - 4);
    }
...
public:
    ...
    if (!send_packet(&pkt, sizeof(pkt), true)) return false;
    // IMPORTANT: don't read the PIT size packet here.
