...
// This list is intentionally conservative and is based on public Heimdall udev
// rules.
static constexpr std::array<uint16_t, 3> SAMSUNG_DOWNLOAD_PIDS{
    {0x6601, 0x685D, 0x68C3}};
...
private:
  ...
  bool odin_send_file_part_and_ack(const unsigned char *data, size_t size,
                                   uint32_t expected_index);
  bool odin_end_sequence_flash(const PitEntry &pit_entry, uint32_t real_size,
                               uint32_t is_last);
  bool odin_send_pit(const std::vector<unsigned char> &pit);
  bool odin_dump_pit(std::vector<unsigned char> &pit_out);
  bool odin_command(uint32_t cmd, uint32_t subcmd, const void *payload,
                    size_t payload_size, std::vector<unsigned char> &rsp,
                    int timeout_ms);
  bool odin_fail_check(const std::vector<unsigned char> &rsp,
                       const std::string &context, bool allow_progress);

  bool bulk_write_all(const void *data, size_t size, int timeout_ms);
  bool bulk_read_once(void *data, size_t size, int *actual_length,
                      int timeout_ms);
...
public:
  ...
  bool receive_packet(void *data, size_t size, int *actual_length,
                      bool is_control = false, size_t min_size = 0,
  ...
  bool flash_partition_stream(std::istream &stream, uint64_t size,
                              const PitEntry &pit_entry, bool large_partition);
  bool send_file_part_chunk(const void *data, size_t size, uint32_t chunk_index,
                            bool large_partition = false);
  ...
  // The returned reference remains valid for the lifetime of the UsbDevice
  // instance.
  ...
  list_download_devices(const UsbSelectionCriteria &criteria);
};
