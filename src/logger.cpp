...
void set_log_file(const std::string &path) {
  ...
  if (!path.empty()) {
    ...
    if (!g_log_stream) {
      std::cerr << timestamp_now()
                << " [ERROR] Unable to open log file: " << path << std::endl;
    }
  }
}
...
void log_warn(const std::string &msg) { log_impl(LogLevel::Warn, msg, false); }

void log_info(const std::string &msg) { log_impl(LogLevel::Info, msg, false); }
