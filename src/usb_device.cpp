  ...
  for (int attempt = 0; attempt < USB_RETRY_COUNT; ++attempt) {
    while (offset < size) {
      ...
      int err = libusb_bulk_transfer(
          handle, endpoint_out, const_cast<unsigned char *>(ptr + offset),
    ...
    if (offset == size) {
      ...
      // Some Odin legacy bootloaders expect it when the transfer size is an
      // exact multiple of wMaxPacketSize.
      if (protocol_mode == ProtocolMode::OdinLegacy && odin_supports_zlp &&
          endpoint_out_max_packet != 0) {
        if ((size % endpoint_out_max_packet) == 0) {
          int zlp_len = 0;
          (void)libusb_bulk_transfer(handle, endpoint_out, nullptr, 0, &zlp_len,
                                     timeout_ms);
        }
      }
      return true;
...
bool UsbDevice::bulk_read_once(void *data, size_t size, int *actual_length,
                               int timeout_ms) {
  for (int attempt = 0; attempt < USB_RETRY_COUNT; ++attempt) {
    int err = libusb_bulk_transfer(
        handle, endpoint_in, static_cast<unsigned char *>(data),
        clamp_size_to_int(size), actual_length, timeout_ms);
    if (err == 0)
      return true;
    if (err == LIBUSB_ERROR_NO_DEVICE)
      return false;
...
static std::string usb_path_for_device(libusb_device *dev) {
  ...
  oss << "/dev/bus/usb/" << std::setfill('0') << std::setw(3)
...
static InterfaceCandidate
find_best_interface(libusb_device *dev, const UsbSelectionCriteria &criteria) {
  ...
  for (int i = 0; i < config->bNumInterfaces; ++i) {
    ...
    for (int j = 0; j < inter->num_altsetting; ++j) {
      ...
      if (criteria.has_interface &&
          id->bInterfaceNumber != criteria.interface_number)
        continue;
...
bool UsbDevice::open_device(const std::string &specific_path,
                            const UsbSelectionCriteria &criteria) {
  ...
  if (!target) {
    last_open_error = saw_candidate_vendor ? UsbOpenError::NotDownloadMode
                                           : UsbOpenError::NoDevice;
    log_warn("No compatible device found. Ensure the device is connected and "
             "in Download Mode.");
    return false;
  }
  ...
  if (libusb_kernel_driver_active(handle, interface_number) == 1) {
    ...
    if (detach_err < 0) {
      last_open_error = (detach_err == LIBUSB_ERROR_ACCESS)
                            ? UsbOpenError::AccessDenied
                            : UsbOpenError::Other;
  ...
  oss << "USB device opened. Path: " << usb_path_for_device(target)
      << ", Interface: " << interface_number << ", EP IN: 0x" << std::hex
      << std::setw(2) << std::setfill('0') << static_cast<int>(endpoint_in)
      << ", EP OUT: 0x" << std::hex << std::setw(2) << std::setfill('0')
      << static_cast<int>(endpoint_out) << std::dec
...
bool UsbDevice::receive_packet(void *data, size_t size, int *actual_length,
                               bool is_control, size_t min_size,
                               int timeout_override_ms) {
  int timeout = timeout_override_ms > 0
                    ? timeout_override_ms
                    : (is_control ? USB_TIMEOUT_CONTROL : USB_TIMEOUT_BULK);
  ...
  for (int attempt = 0; attempt < USB_RETRY_COUNT; ++attempt) {
    ...
    *actual_length = 0;
    ...
    while (received < required_min && received < size) {
      ...
      int err = libusb_bulk_transfer(
          handle, endpoint_in, static_cast<unsigned char *>(data) + received,
      ...
      log_error("USB packet receive failed (attempt " +
    ...
    log_error("Incorrect receive size (attempt " + std::to_string(attempt + 1) +
              "): expected min " + std::to_string(required_min) + ", max " +
              std::to_string(size) + ", received " + std::to_string(received));
...
bool UsbDevice::request_device_type() {
  ...
  int actual_length;
  if (!receive_packet(&response, sizeof(response), &actual_length, true,
                      sizeof(response)))
    return false;
...
UsbDevice::list_download_devices(const UsbSelectionCriteria &criteria) {
...
bool UsbDevice::end_session() {
  ...
  if (actual_length < static_cast<int>(sizeof(ThorResponsePacket))) {
    log_error("Session end failed: short response (" +
              std::to_string(actual_length) + " bytes)");
    return false;
  }
...
static bool parse_pit_bytes(PitTable &pit_table,
                            const std::vector<unsigned char> &pit_data) {
  if (pit_data.size() < 28) {
  ...
  if (pit_table.entry_count == 0 || pit_table.entry_count > 512) {
    log_error("Invalid PIT entry count: " +
              std::to_string(pit_table.entry_count));
    return false;
  }
  ...
  const size_t required =
      pit_table.header_size +
      static_cast<size_t>(pit_table.entry_count) * entry_size;
  if (pit_data.size() < required) {
    log_error("PIT truncated: expected at least " + std::to_string(required) +
              " bytes");
    return false;
  }
  ...
  for (uint32_t i = 0; i < pit_table.entry_count; ++i) {
    const size_t off =
        pit_table.header_size + static_cast<size_t>(i) * entry_size;
    PitEntry e = {};
    ...
    if (part_name.empty()) {
      log_error("PIT entry " + std::to_string(i) +
                " has an empty partition name");
      return false;
    }
    if (!is_valid_pit_string(part_name)) {
      log_error("PIT entry " + std::to_string(i) +
                " has an invalid partition name: '" + part_name + "'");
      return false;
    }
    if (!file_name.empty() && !is_valid_pit_string(file_name)) {
      log_error("PIT entry " + std::to_string(i) +
                " has an invalid file name: '" + file_name + "'");
      return false;
    }

    if (e.identifier == 0) {
      log_error("PIT entry " + std::to_string(i) +
                " has an invalid identifier (0)");
      return false;
    }
    if (!seen_identifiers.insert(e.identifier).second) {
      log_error("PIT contains duplicate partition identifier: " +
                std::to_string(e.identifier));
      return false;
    }
    ...
    if (e.block_count != 0) {
      ...
      if (a != 0 && prod / a != b) {
        log_error("PIT entry " + std::to_string(i) +
                  " has an overflow in block size/count");
        return false;
      }
    }
...
bool UsbDevice::receive_pit_table(PitTable &pit_table) {
  ...
  int actual_length = 0;
  if (!receive_packet(&pit_size_pkt, sizeof(pit_size_pkt), &actual_length, true,
                      sizeof(pit_size_pkt)))
    return false;

  if (actual_length < static_cast<int>(sizeof(ThorPitFilePacket))) {
    log_error("Short PIT size packet (" + std::to_string(actual_length) +
              " bytes)");
    return false;
  }
  ...
  if (!receive_packet(pit_data.data(), pit_data_size, &actual_length, false,
                      pit_data_size)) {
...
bool UsbDevice::send_file_part_chunk(const void *data, size_t size,
  ...
  if (actual_length < static_cast<int>(sizeof(ThorResponsePacket))) {
    log_error("File part control failed: short response (" +
              std::to_string(actual_length) + " bytes)");
    return false;
  }
  ...
  // Always wait for the post-data ACK, with a reasonable timeout, to avoid
  // leaving it queued in the IN endpoint.
  ThorResponsePacket post = {};
  int post_len = 0;
  int post_timeout = large_partition ? 30000 : 10000;
  if (!receive_packet(&post, sizeof(post), &post_len, true,
                      sizeof(ThorPacketHeader), post_timeout)) {
    log_error("Timed out waiting for post-data ACK");
    return false;
  }
  if (post_len < static_cast<int>(sizeof(ThorResponsePacket))) {
    log_error("Post-data ACK was short (" + std::to_string(post_len) +
              " bytes)");
    return false;
  }
  uint32_t post_code = le32toh(post.response_code);
  if (le16toh(post.header.packet_type) != THOR_PACKET_RESPONSE ||
      post_code != 0) {
    log_error("File part data ACK reported failure. Code: " +
              std::to_string(post_code));
    return false;
  }
...
bool UsbDevice::send_file_part_header(uint64_t total_size) {
  ...
  if (le16toh(response.header.packet_type) != THOR_PACKET_RESPONSE ||
      code != 0) {
    log_error("Unexpected response sending file part size. Code: " +
              std::to_string(code));
    return false;
  }
  return true;
...
bool UsbDevice::end_file_transfer(uint32_t partition_id) {
  ...
  log_info("Finalizing file transfer for partition ID: " +
  ...
  if (actual_length < static_cast<int>(sizeof(ThorResponsePacket))) {
    log_error("File transfer finalization failed: short response (" +
              std::to_string(actual_length) + " bytes)");
    return false;
  }
  uint32_t code = le32_to_h(response.response_code);
  if (le16toh(response.header.packet_type) != THOR_PACKET_RESPONSE ||
      code != 0) {
    log_error("File transfer finalization failed. Code: " +
              std::to_string(code));
    return false;
  }
...
bool UsbDevice::odin_command(uint32_t cmd, uint32_t subcmd, const void *payload,
                             size_t payload_size,
...
bool UsbDevice::odin_fail_check(const std::vector<unsigned char> &rsp,
  ...
  if (allow_progress) {
    // Some bootloaders report intermediate/progress states using 0xFF + a
    // negative code. Treat those as non-fatal when explicitly allowed.
    if (code >= -7 && code <= -2) {
...
bool UsbDevice::odin_legacy_handshake() {
  ...
  if (actual != 4)
    return false;
  if (!(reply[0] == 'L' && reply[1] == 'O' && reply[2] == 'K' &&
        reply[3] == 'E'))
    return false;
  return true;
}
...
bool UsbDevice::odin_begin_session() {
  ...
  if (version <= 1) {
    ...
    uint32_t packet_size =
        h_to_le32(static_cast<uint32_t>(odin_flash_packet_size));
...
bool UsbDevice::odin_send_file_part_and_ack(const unsigned char *data,
                                            size_t size,
  ...
  if (static_cast<uint32_t>(idx) != expected_index) {
    log_error("Bootloader file part index mismatch: expected " +
              std::to_string(expected_index) + " got " + std::to_string(idx));
    return false;
  }
  return true;
}

bool UsbDevice::odin_end_sequence_flash(const PitEntry &pit_entry,
                                        uint32_t real_size, uint32_t is_last) {
  std::vector<unsigned char> rsp;
...
bool UsbDevice::flash_partition_stream(std::istream &stream, uint64_t size,
                                       const PitEntry &pit_entry,
                                       bool large_partition) {
  ...
  if (protocol_mode == ProtocolMode::OdinLegacy) {
    ...
    const uint64_t sequence_bytes =
        static_cast<uint64_t>(odin_flash_packet_size) *
        static_cast<uint64_t>(odin_flash_sequence_count);
    if (sequence_bytes == 0)
      return false;
    ...
    uint64_t last_sequence64 =
        size - static_cast<uint64_t>(sequences - 1) * sequence_bytes;
    if (last_sequence64 == 0)
      last_sequence64 = sequence_bytes;
    if (last_sequence64 > 0xFFFFFFFFull) {
      log_error("Legacy last sequence too large: " +
                std::to_string(last_sequence64));
      return false;
    }
    const uint32_t last_sequence = static_cast<uint32_t>(last_sequence64);

    uint64_t total_sent = 0;
    std::vector<unsigned char> part(static_cast<size_t>(odin_flash_packet_size),
                                    0);
    ...
    for (uint32_t i = 0; i < sequences; ++i) {
      ...
      const uint32_t real_size =
          last ? last_sequence : static_cast<uint32_t>(sequence_bytes);
      uint32_t aligned_size = real_size;
      if (aligned_size % static_cast<uint32_t>(odin_flash_packet_size) != 0) {
        aligned_size +=
            static_cast<uint32_t>(odin_flash_packet_size) -
      ...
      for (uint32_t j = 0; j < parts; ++j) {
        ...
        if (!odin_send_file_part_and_ack(part.data(), part.size(),
                                         expected_index++))
          return false;
  ...
  while (remaining > 0) {
    ...
    if (!send_file_part_chunk(buf.data(), to_read, chunk_index,
                              large_partition))
      return false;
    remaining -= to_read;
    chunk_index++;
