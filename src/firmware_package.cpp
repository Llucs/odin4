...
namespace {
    ...
    ExitCode detect_tar_md5_info(const std::string& file_path, TarMd5Info& info) {
        ...
        for (int64_t pos = static_cast<int64_t>(tail.size()) - 32; pos >= 0; --pos) {
            ...
            const bool right_ok = (static_cast<size_t>(pos + 32) >= tail.size()) || !is_hex_char(static_cast<unsigned char>(tail[static_cast<size_t>(pos + 32)]));
    ...
    ExitCode verify_tar_md5(const std::string& file_path, const TarMd5Info& info) {
        if (!info.has_md5) return ExitCode::Success;
...
bool check_md5_signature(const std::string& file_path) {
    ...
    if (di != ExitCode::Success) return false;
...
bool process_lz4_streaming(std::ifstream& file, uint64_t compressed_size, UsbDevice& usb_device, const std::string& filename, bool large_partition, bool do_flash) {
    ...
    if (uncompressed_size == 0) {
        ...
        while (scan_remaining > 0) {
            ...
            while (src_off < read) {
                ...
                scan_err = LZ4F_decompress(scan_ctx, out_buf.data(), &dst_sz, in_buf.data() + src_off, &src_sz, nullptr);
                if (LZ4F_isError(scan_err)) {
        ...
        while (src_size > 0) {
            ...
            if (dst_size > 0) {
                ...
                if (show_progress && uncompressed_size > 0) {
                    const int percent = static_cast<int>((static_cast<double>(total_uncompressed) / static_cast<double>(uncompressed_size)) * 100.0);
    ...
    if (uncompressed_size != 0 && total_uncompressed != uncompressed_size) {
        log_error("Decompressed size mismatch for " + filename + ": expected " + std::to_string(uncompressed_size) + ", got " + std::to_string(total_uncompressed));
        return false;
    }
...
ExitCode process_tar_file(const std::string& tar_path, UsbDevice& usb_device, const PitTable& pit_table, bool do_flash, bool allow_unknown) {
    ...
    while (true) {
        ...
        if (!pending_gnu_long_name.empty()) {
            ...
            if (std::memcmp(header + 257, "ustar", 5) == 0) {
                ...
                if (!prefix.empty()) filename = prefix + "/" + name;
                else filename = name;
            } else {
                filename = name;
            }
        ...
        const uint64_t data_start = static_cast<uint64_t>(file.tellg());
        if (data_start + data_size > content_end) {
        ...
        if (typeflag == 'L') {
            ...
            if (nul != std::string::npos) long_name = long_name.substr(0, nul);
        ...
        for (const auto& entry : pit_table.entries) {
            ...
            if (!pit_file.empty() && (pit_file_s == base_name_sanitized || to_lower_copy(pit_file) == to_lower_copy(basename))) {
                pit_entry = &entry;
        ...
        if (!used_partition_ids.insert(pit_entry->identifier).second) {
            log_error("Archive contains multiple entries for the same PIT partition identifier: " + std::to_string(pit_entry->identifier));
            return ExitCode::Pit;
        }
        ...
        log_verbose("TAR entry: " + filename + " (" + std::to_string(data_size) + " bytes) -> PIT: " + partition_name + " (ID " + std::to_string(pit_entry->identifier) + ")");
