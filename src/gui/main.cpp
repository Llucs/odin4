...
class OdinGui : public QMainWindow {
        ...
        auto createBrowseRow = [&](const QString& label, QLineEdit*& edit) {
            ...
            connect(btn, &QPushButton::clicked, [this, edit]() {
                QString file = QFileDialog::getOpenFileName(this, "Select File", "",
                                                            "Samsung Firmware (*.tar *.tar.md5 *.lz4 *.bin)");
                if (!file.isEmpty())
                    edit->setText(file);
            });
            row->addWidget(btn, 0);
    ...
    void closeEvent(QCloseEvent* event) override {
        if (flashThread && flashThread->isRunning()) {
            auto ret = QMessageBox::question(this, "Exit",
                                             "A flash process is currently running. Are you sure you want to exit?",
  ...
  private slots:
    void refreshDevices() {
        ...
        if (devices.empty())
            comboDevices->addItem("No device detected");
    }
    ...
    void onFlashFinished(OdinExitCode result) {
        ...
        if (result == OdinExitCode::Success) {
            ...
            logView->append(
                QString("<span style='color:red;'>Flash failed with code: %1</span>").arg(static_cast<int>(result)));
